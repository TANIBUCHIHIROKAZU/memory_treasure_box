class Customer < ApplicationRecord
  ...
  def follow(other_customer_id)
    unless self == other_customer_id
      connections.find_or_create_by(follow_id: other_customer_id)
    end
  end

  # フォローを外す
  def unfollow(other_customer_id)
    connection = connections.find_by(follow_id: other_customer_id)
  ...
  def follow_only
    followings.where.not(id: followers.ids)
  end

  # 相手はフォローしているが、自分はフォローしていない
  def follower
    followers.where.not(id: followings.ids)
  end
